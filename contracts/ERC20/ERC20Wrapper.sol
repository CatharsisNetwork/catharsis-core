...
contract ERC20Wrapper {
    ...
    address public immutable fractionalToken;

    constructor(IFractionalToken _fractionalToken) {
        fractionalToken = address(_fractionalToken);
    }
    ...
    function issue(
        ...
        require(
            getWrappedToken[fractionalToken][_tokenId] == address(0),
            "Wrapped token already issued"
        );

        bytes memory bytecode = type(WrappedFractionalToken).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(fractionalToken, _tokenId));
        assembly {
            wrappedToken := create2(0, add(bytecode, 0x20), mload(bytecode), salt)
        }

        getWrappedToken[fractionalToken][_tokenId] = wrappedToken;
    }
}